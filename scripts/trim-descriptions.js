...
const MAX = { ja: 95, zh: 95, 'zh-hant': 95 }, DEFAULT_MAX = 160, MIN = { ja: 45, zh: 45, 'zh-hant': 45 }, DEFAULT_MIN = 120;
const files = (langs.length ? langs : fs.readdirSync(LOC).map(f => path.basename(f, '.json'))).sort();
let changed = 0;
for (const code of files) {
  const file = path.join(LOC, code + '.json'); let text = fs.readFileSync(file, 'utf8'); const L = JSON.parse(text);
  ...
  for (const [name, get, put] of set) {
    ...
    if (after !== before) { changed++; console.log(`${code} ${name}: ${before.length} → ${after.length}`); if (write) { put(after); text = text.replace(JSON.stringify(before), () => JSON.stringify(after)); } }
  }
  if (write) fs.writeFileSync(file, text); // точечная замена строк: форматирование файла не меняется
}
